...
export default function EditBanner({ bannerEditId, banners, setShowEditCategoryComponent }) {
  ...
  const onSubmit = async (e) => {
    ...
    if (imageInputNew.length > 0) {
      imageInput.forEach(item => {
        const desertRef = ref(storage, `bannerPrincipal/${item.path}`);
      ...
      imageInputNew.forEach(async item => {
        ...
        setTimeout(async () => {
          if (imageInput.length === contador) {
            contador = 0;
            let data = await axios.put(`/api/banners/updateBanners?id=${id_}`, {
              title: bannerTitle,
              text: bannerText,
