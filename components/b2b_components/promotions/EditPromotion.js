...
export default function EditPromotion({ promotionEditId, promotionsArr, setShowEditCategoryComponent }) {
  ...
  const onSubmit = async (e) => {
    ...
    if (imageInputNew.length > 0) {
      imageInput.forEach(item => {
        const desertRef = ref(storage, `bannerPromocoes/${item.path}`);
      ...
      imageInputNew.forEach(async item => {
        ...
        setTimeout(async () => {
          if (imageInput.length === contador) {
            contador = 0;
            let data = await axios.put(`/api/promotions/updatePromotions?id=${id_}`, {
